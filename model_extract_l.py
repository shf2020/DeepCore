...
BATCH_SIZE = 512
EPOCH = 100
dir ='extract_l_models_tinyImageNet'
...
def train_student_model(iter,teacher,cls):
    ...
    for epoch in range(EPOCH):
        ...
        if accu1 > accu_best:
            torch.save(model.state_dict(), os.path.join(dir, "extract_model_l_" + str(cls) + "_" + str(iter) + ".pth"))
            accu_best = accu1
...
if __name__ == "__main__":
    ...
    f = open(os.path.join(dir, "extract_model_vgg16_accus.txt"),mode='w')
    f.write("acc:"+" \n".join(map(str,accus)))
    f.close()
    print(accus)
    # iter = 1
